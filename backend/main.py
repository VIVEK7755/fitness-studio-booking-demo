...
from backend.models import Class, ClassRead, Booking, BookingRequest, BookingRead
...
def book_class(booking: BookingRequest, session: Session = Depends(get_session)):
...
def get_bookings(
    email: Optional[EmailStr] = Query(None, description="Client email to filter bookings"),
